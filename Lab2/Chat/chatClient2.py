...
class ServerPart:
    ...
    def run(self):
        try:
            while True:
                ConnectionSocket, clientAddr = self.wait_accept()
                t = Thread(target=self.handle_request, args=(ConnectionSocket, clientAddr))
                t.start()
        except:
